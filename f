...
Z_SHORT = 0.01
 # Weight for short reading
Z_MAX = 0.17 # Weight for max reading
...
SIGMA_HIT = 0.05 # Noise value for hit reading
LAMBDA_SHORT = 0.1
...
class SensorModel:
  ...
  #Apply normal distribute to data
  def calculate_hit_probability(self,z_tk, z_star_tk, sigma_hit):
   
    prob_hit = (1 / (sigma_hit * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((z_tk - z_star_tk) ** 2 / sigma_hit ** 2))
    return prob_hit

  def calculate_short_probability(self,z_tk, z_star_tk, lambda_short):
   
    prob_short = (lambda_short * np.exp(-lambda_short * z_tk)) if z_tk < z_star_tk else 0
    return prob_short

  def calculate_max_probability(self,z_tk, z_max):
   
    prob_max = 1 / z_max if z_tk == z_max else 0
    return prob_max

  def calculate_rand_probability(self,z_tk, z_max):
  ...
  def precompute_sensor_model(self, max_range_px):
    ...
    for true_range in range(table_width):
      for sim_range in range(table_width):
        ...
        p_hit = self.calculate_hit_probability(sim_range, true_range, SIGMA_HIT)
        p_short = self.calculate_short_probability(sim_range, true_range, LAMBDA_SHORT)
        p_max = self.calculate_max_probability(sim_range, Z_MAX)
        p_rand = self.calculate_rand_probability(sim_range, Z_MAX)
        ...
        sensor_model_table[sim_range, true_range] = Z_HIT * norm * p_hit + Z_SHORT * norm * p_short + Z_MAX * p_max + Z_RAND * p_rand
...
On Wed, Nov 22, 2023 at 7:34 PM Arnab Karmakar <arnabk1@uw.edu> wrote:
The existing code in github might crash. Use this sensormodel code instead.

Record a video and put it in github writeup folder if it works perfectly
